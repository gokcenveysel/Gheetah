...
{
    public static class JavaScenarioExecutor
    {
        public static async Task ExecuteAsync(string projectPath, string scenarioTag, string processId)
        {
            bool xmlReportGenerated = false;
            string zipFilePath = projectPath + ".zip";
            try
            {
                StatusUI.ShowStatus($"Entering JavaScenarioExecutor.ExecuteAsync: ProjectPath={projectPath}, ScenarioTag={scenarioTag}, ProcessId={processId}");
                LogToFile($"Entering JavaScenarioExecutor.ExecuteAsync: ProjectPath={projectPath}, ScenarioTag={scenarioTag}, ProcessId={processId}");
                ...
                if (pomFiles.Length > 0)
                {
                    command = $@"cd '{buildDir}'; mvn test `-Dcucumber.filter.tags=""@{scenarioTag}""";
                }
                else if (gradleFiles.Length > 0)
                {
                    command = $@"cd '{buildDir}'; gradle clean test `-Pcucumber.filter.tags=""@{scenarioTag}""";
                }
                else
                {
                    StatusUI.ShowStatus($"Error: No pom.xml or build.gradle file found in {projectPath} or subdirectories");
                ...
                try
                {
                    ...
                    {
                        var xmlFiles = Directory.GetFiles(testResultsDir, $"{scenarioTag}_*_test_results.xml")
                ...
                {
                    StatusUI.ShowStatus($"Error: XML report not found in {testResultsDir} for tag {scenarioTag}");
                    LogToFile($"Error: XML report not found in {testResultsDir} for tag {scenarioTag}");
                    await SendOutputWithTimeout($"Error: XML report not found in {testResultsDir} for tag {scenarioTag}", processId);
                }
            }
            catch (Exception ex)
            {
