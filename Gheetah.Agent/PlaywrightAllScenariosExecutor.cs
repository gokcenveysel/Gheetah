...
{
    public static class PlaywrightAllScenariosExecutor
    {
...
";
        ...
        {
            StatusUI.ShowStatus($"Executing all Playwright tests: ProjectPath={projectPath}, ProcessId={processId}");
            ...
            try
            {
                ...
                string command = $@"cd '{projectDir}'; npx playwright test --config {GheetahConfigName}";
            ...
            {
                StatusUI.ShowStatus($"Error executing all Playwright tests: {ex.Message}");
                await AgentService.SendOutputAsync($"Error executing all Playwright tests: {ex.Message}", processId);
                await AgentService.SendResultAsync($"Error:All Playwright tests execution failed:{ex.Message}", processId);
            }
            finally
            {
