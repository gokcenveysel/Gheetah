...
{
    public class GrokAgentAdapter : IAiAgentAdapter
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private readonly ILogger<GrokAgentAdapter> _logger;

        public string ProviderType => "Grok";

        public GrokAgentAdapter(IHttpClientFactory httpClientFactory, ILogger<GrokAgentAdapter> logger)
        {
        ...
        public async Task<bool> IsHealthyAsync(AiAgent agent)
        {
            try
            {
                ...
                var response = await client.GetAsync("https://api.x.ai/v1/models");
                return response.IsSuccessStatusCode;
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Grok health check failed for agent {AgentId}", agent.Id);
                return false;
            }
        }
        ...
        public async Task<string> GenerateAsync(AiAgent agent, string prompt, CancellationToken ct = default)
        {
            ...
            var body = new
            {
                model = agent.ModelName ?? "grok-2",
                messages = new[] { new { role = "user", content = prompt } }
            };
            var content = new StringContent(JsonSerializer.Serialize(body), Encoding.UTF8, "application/json");
            var response = await client.PostAsync("https://api.x.ai/v1/chat/completions", content, ct);
